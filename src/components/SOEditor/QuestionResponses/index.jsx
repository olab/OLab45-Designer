...
class QuestionResponses extends ScopedObjectService {
  ...
  buildId = (name: String, index: Number): void =>
    `${name.toLowerCase()}-${index}`;

  setPageTitle = (): void => {
    document.title = 'Edit Question Responses';
  };
  ...
  isDataChanged = (): void => {
    ...
    const changed = responses.filter(
      (value) =>
        value.dbOperation === 'delete' ||
        value.dbOperation === 'edit' ||
        value.dbOperation === 'create',
    );
  ...
  render() {
    const { isDetailsFetching, isFieldsDisabled, id, responses } = this.state;
