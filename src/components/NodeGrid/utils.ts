...
export const getNodesReduced = (
  propsNodes: Array<FullNode>,
): NodeGridState => ({
  nodes: propsNodes.map(({ id, title, text, x, y }: NodeType): NodeType => ({
    x: parseInt(x, 10),
    y: parseInt(y, 10),
    id,
    title,
...
export const withoutTags = (field: string | number): string | number =>
  isString(field) ? removeHTMLTags(field) : field;

export const sortNodesByField =
  (field: string, status: string): Function =>
  (a: NodeType, b: NodeType): number => {
    switch (status) {
      case ORDER.DESC:
...
export const unEscapeHtml = (data: string): string =>
  data
    .replace(/&lt;/g, '<')
    .replace(/&gt;/g, '>')
    .replace(/&amp;/g, '&')
