...
import type {
  NodeGridTableProps as IProps,
  NodeGridTableState as IState,
} from './types';
...
export class NodeGridTable extends PureComponent<IProps, IState> {
  ...
  handleChange =
    (nodeKey: string, index: number): Function =>
    (html: string): void => {
      ...
      const resultNodes = [
        ...
        {
          ...nodes[index],
          [nodeKey]: ['x', 'y'].includes(nodeKey) ? Number(html) : html,
        },
        ...nodes.slice(index + 1),
      ];
  ...
  handleSort =
    (headLabelKey: string): Function =>
    (): void => {
      const { sortStatus: stateSortStatus } = this.state;
      const { nodes, onTableChange } = this.props;
      const sortedNodes = nodes.sort(
        sortNodesByField(headLabelKey, stateSortStatus[headLabelKey]),
      );
      ...
      this.setState(({ sortStatus }): IState => ({
        sortStatus: {
          ...sortStatus,
          [headLabelKey]:
            sortStatus[headLabelKey] === ORDER.ASC ? ORDER.DESC : ORDER.ASC,
        },
        headLabelKey,
      }));
        ...
        <Table>
          <TableHead>
            <TableRow className={classes.tableRow}>
              {Object.keys(TABLE_HEAD_CELLS).map(
                (headLabelKey: string): React$Element<any> => {
                  const { isSortable } = TABLE_HEAD_CELLS[headLabelKey];
                  const { label } = TABLE_HEAD_CELLS[headLabelKey];
                  const onClickHandler = isSortable
                    ? this.handleSort(headLabelKey)
                    : null;
          ...
          <TableBody>
            {nodes.map((node: NodeType, i: number): React$Element<any> => (
              ...
              >
                {Object.keys(TABLE_HEAD_CELLS).map(
                  (nodeKey: string): React$Element<any> => {
